...
# NOTE: this file is the self-contained chemistry-annotation counterpart to
# gnn_parts.py. It imports from gpmhc.data (unmodified, shared by both
# pipelines - it only defines tokenization, not graph/edge logic) and from
# the new gpmhc.chemistry_features module. It does NOT import anything from
# gpmhc.gnn_parts, and gpmhc.gnn_parts is never imported by this file or by
# baseline_model_chemannot.py - the two pipelines share no code path beyond
# gpmhc.data.
from gpmhc.data import categories, categories_val
from gpmhc.chemistry_features import get_chemistry_features, CHEMISTRY_FEATURE_NAMES

# Reverse mapping from tokenized integer id back to the single-character
# residue code, built once from data.py's existing categories/categories_val
# (the same tables used to numericalize sequences in tokenize()). Used only
# to look up residue identities for chemistry-feature annotation - does not
# affect tokenization, node features, or graph topology in any way.
# Reverse mapping from tokenized integer id back to the single-character
# residue code, built once from data.py's existing categories/categories_val
# (the same tables used to numericalize sequences in tokenize()). Used only
# to look up residue identities for chemistry-feature annotation - does not
# affect tokenization, node features, or graph topology in any way.
TOKEN_TO_RESIDUE = {int(tok): letter for tok, letter in zip(categories_val, categories)}
N_CHEMISTRY_FEATURES = len(CHEMISTRY_FEATURE_NAMES)

# Vectorization: measured logs showed dgl.batch() at ~0.01-0.02s/batch vs.
# ~2.3-3.5s/batch for the original per-edge Python loop over 130k-170k edges
# - a ~150-500x gap, confirming the loop (not graph batching) as the actual
# bottleneck. Fix: precompute chemistry features for every possible
# (token, token) pair ONCE at module import time (the vocabulary here is
# small - ~28 symbols per data.py's `categories` - so this is a 28x28x6
# table, 784 total calls to get_chemistry_features(), done once), then at
# call time reduce annotation to a single vectorized tensor index instead
# of a Python loop. The feature DEFINITIONS are unchanged - this table is
# built by calling the exact same get_chemistry_features() function from
# chemistry_features.py that the original loop called per-edge; only the
# mechanism (precomputed table + vectorized lookup vs. repeated per-edge
# Python calls) changes.
_MAX_TOKEN_ID = max(TOKEN_TO_RESIDUE.keys())
_CHEM_TABLE = torch.zeros(
    (_MAX_TOKEN_ID + 1, _MAX_TOKEN_ID + 1, N_CHEMISTRY_FEATURES), dtype=torch.float32
)
for _t1, _r1 in TOKEN_TO_RESIDUE.items():
    for _t2, _r2 in TOKEN_TO_RESIDUE.items():
        _CHEM_TABLE[_t1, _t2] = torch.tensor(
            get_chemistry_features(_r1, _r2), dtype=torch.float32
        )
# Any token id not present in TOKEN_TO_RESIDUE (shouldn't occur given how
# node_tokens is derived from the same tokenizer, but guarded defensively)
# falls back to an all-zero row via the table's initialization above.


def compute_chemistry_edge_features(u, v, node_tokens):
    """Append-only chemistry annotation for EXISTING edges.

    Parameters
    ----------
    u, v : 1D long tensors of node indices (an existing graph's edge list,
        as returned by graph.edges() - not modified, not reordered, not
        filtered here)
    node_tokens : 1D long tensor of raw (pre-embedding) residue token ids,
        aligned to the same node ordering as u/v and as the node feature
        tensor already used elsewhere in this pipeline.

    Returns
    -------
    float32 tensor of shape (E, 6), one row per existing edge, in the same
    order as u/v. Computed via a single vectorized index into a precomputed
    (token, token) -> chemistry-feature table - see _CHEM_TABLE above.
    Replaces an earlier per-edge Python loop that was measured to be the
    dominant per-batch cost (2.3-3.5s vs. ~0.01-0.02s for dgl.batch() on the
    same batches).
    """
    tok_u = node_tokens[u].long()
    tok_v = node_tokens[v].long()
    return _CHEM_TABLE[tok_u, tok_v]
...
def lookup_graph(features, features_token, lengths, allele, lookup_table, bc_pad, use_chemistry_edges=True):
    import time
    _t0 = time.perf_counter()
    print(f"[lookup_graph] START batch_size={len(lengths[1])}", flush=True)
    allele = allele.cpu()
    ...
    # Retained (unlike gnn_parts.py's original, which discards it) so
    # chemistry annotation can map surviving nodes back to residue identity.
    # Same filtering, same ordering as `features` above.
    features_token_filtered = features_token[nonzero].squeeze()
    ...
    print(f"[lookup_graph] after dgl.batch(): {_t_batch_end - _t_batch_start:.3f}s  "
          f"num_graphs={len(condition_index)}  num_nodes={graph.num_nodes()}  "
          f"num_edges={graph.num_edges()}  (cumulative {_t_batch_end - _t0:.3f}s)", flush=True)
    ...
    if use_chemistry_edges:
        # Append-only: reads back the exact (u, v) pairs already baked into
        # the batched graph object - no re-derivation or reordering of edges.
        print(f"[lookup_graph] before chemistry annotation: num_edges={graph.num_edges()}", flush=True)
        _t_chem_start = time.perf_counter()
        u_idx, v_idx = graph.edges()
        chem = compute_chemistry_edge_features(u_idx, v_idx, features_token_filtered)
        edge = torch.cat([edge.float(), chem], dim=1)
        _t_chem_end = time.perf_counter()
        print(f"[lookup_graph] after chemistry annotation: {_t_chem_end - _t_chem_start:.3f}s  "
              f"(cumulative {_t_chem_end - _t0:.3f}s)", flush=True)

    print(f"[lookup_graph] END total={time.perf_counter() - _t0:.3f}s", flush=True)
    return graph, features, edge, index

#lookup one graph per data after best allele/binding core start position is known
def lookup_single_graph(features, features_token, lengths, allele, lookup_table, positions, use_chemistry_edges=True):
    import time
    _t0 = time.perf_counter()
    print(f"[lookup_single_graph] START batch_size={len(lengths[1])}", flush=True)
    allele=allele.cpu()
    features = features.cpu()
    features_token = features_token.cpu()
    for ix,length in enumerate(lengths):
    ...
    graph = dgl.batch(graph)
    _t_batch_end = time.perf_counter()
    print(f"[lookup_single_graph] after dgl.batch(): {_t_batch_end - _t_batch_start:.3f}s  "
          f"num_graphs={len(condition_index)}  num_nodes={graph.num_nodes()}  "
          f"num_edges={graph.num_edges()}  (cumulative {_t_batch_end - _t0:.3f}s)", flush=True)
    ...
    # Retained (unlike gnn_parts.py's original) so chemistry annotation can
    # map surviving nodes back to residue identity.
    features_token_filtered = features_token.view(-1)[nonzero]

    if use_chemistry_edges:
        print(f"[lookup_single_graph] before chemistry annotation: num_edges={graph.num_edges()}", flush=True)
        _t_chem_start = time.perf_counter()
        u_idx, v_idx = graph.edges()
        chem = compute_chemistry_edge_features(u_idx, v_idx, features_token_filtered)
        edge = torch.cat([edge.float(), chem], dim=1)
        _t_chem_end = time.perf_counter()
        print(f"[lookup_single_graph] after chemistry annotation: {_t_chem_end - _t_chem_start:.3f}s  "
              f"(cumulative {_t_chem_end - _t0:.3f}s)", flush=True)

    print(f"[lookup_single_graph] END total={time.perf_counter() - _t0:.3f}s", flush=True)
