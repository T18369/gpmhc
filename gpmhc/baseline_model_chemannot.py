...
from gpmhc.gnn_parts_chemannot import *
...
class model(ModelRequirements):
    ...
    def get_model(self, model_hyperparameters):
        ...
        gnn = GNN(
            ...
            # This pipeline is chemistry-annotation-only by design (a separate
            # file from the baseline, per the non-destructive-parallel-
            # implementation approach) - defaults True but still reads from
            # config so json_input_chemannot.json is the single source of truth.
            use_chemistry_edges=bool(model_hyperparameters.get("use_chemistry_edges", True)),
        )
        model = gnn
        model.arch = self
        return model
...
class GNN(nn.Module):
    def __init__(
        ...
        self.use_chemistry_edges = use_chemistry_edges
    ...
    def forward(self, x_data):
        ...
        g, node_feats, edge_feats, index = lookup_graph(feat, feat_token, lens, alleles, self.lookup_table, self.bc_pad, use_chemistry_edges=self.use_chemistry_edges)
        ...
        g, node_feats, edge_feats = lookup_single_graph(feat, feat_token, lens, alleles, self.lookup_table, graph_idx.cpu(), use_chemistry_edges=self.use_chemistry_edges)
        node_feats = node_feats.cuda()
